...
X = dataset.iloc[:, [3, 4]].values # include more column if have
...
dendrogram = sch.dendrogram(sch.linkage(X, method = 'ward'))#ward minimise variance in each cluster
...
plt.scatter(X[y_hc == 0, 0], X[y_hc == 0, 1], s = 100, c = 'red', label = 'Cluster 1')# here we are using y_hc instead of k_mean in kmean function
